...
import 'tabs/auto_orders_tab.dart';
...
class _AdminShellState extends State<AdminShell> {
  ...
  static const _tabs = [
    ...
    _TabInfo(icon: Icons.flash_on_rounded, label: 'Otomatik Siparişler'),
  ...
  List<int> _visibleTabIndexes(AuthProvider auth) {
    ...
    // 3 = Ayarlar, 10 = Güvenlik (Otomatik Siparişler tabı eklenince Güvenlik 9'dan 10'a kaydı)
    return all.where((index) => index != 3 && index != 10).toList();
  }

  List<int> _bottomTabIndexes(List<int> visibleTabIndexes) {
    // 0=Dashboard, 5=Siparişler, 6=Otomatik Siparişler, 7=Kullanıcılar, 8=Ödemeler, 9=Destek
    const preferred = <int>[0, 5, 6, 7, 8, 9];
    final indexes =
  ...
  void _switchTab(int index) {
    ...
    // Otomatik Siparişler tabı eklenince Destek tab'ı 8'den 9'a kaydı.
    final notifService = SupportNotificationService.instance;
    notifService.supportTabActive = index == 9;
    if (index == 9) {
