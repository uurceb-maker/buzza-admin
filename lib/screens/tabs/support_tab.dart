...
class _SupportTabState extends State<SupportTab> {
  ...
  void _onReplyChanged() {
    ...
    if (now.difference(_lastTypingSentAt).inMilliseconds < 3000) return;
    _lastTypingSentAt = now;
    _typingDebounce?.cancel();
    _typingDebounce = Timer(const Duration(milliseconds: 200), () {
      // En iyi-çaba: hata sessizce yutulur (UI'ı bloklamaz).
      unawaited(_api.sendSupportTypingSignalV2(id).catchError((_) =>
          <String, dynamic>{}));
    });
  }
  ...
  void _toggleSelected(int ticketId) {
    setState(() {
      if (_selectedIds.contains(ticketId)) {
  ...
  Future<void> _confirmBulkDelete() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Seçili biletleri sil'),
        content: Text(
          '$count bilet ve onlara ait tüm mesajlar/ekler kalıcı olarak silinecek. '
    ...
    try {
      ...
      await _api.bulkDeleteSupportTickets(ids);
      // Aktif bilet de silindiyse temizle.
      if (ids.contains(_toInt(_active?['id']))) {
        _threadPoll?.cancel();
        _active = null;
      ...
      _snack('$count bilet silindi.', AppTheme.success);
      await _loadTickets(showLoader: false, fromPolling: false);
    } catch (e) {
      _snack('Silme başarısız: $e', AppTheme.error);
    } finally {
  ...
  Future<void> _confirmDeleteMessage(int messageId) async {
    final ticketId = _toInt(_active?['id']);
    if (ticketId <= 0 || messageId <= 0) return;
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Mesajı sil'),
        content: const Text(
          'Bu mesaj ve ekleri kalıcı olarak silinecek. Bu işlem geri alınamaz.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(ctx).pop(false),
    ...
    try {
      await _api.deleteSupportMessages(ticketId, [messageId]);
      _snack('Mesaj silindi.', AppTheme.success);
      await _refreshActive(fromPolling: false);
    } catch (e) {
      _snack('Mesaj silinemedi: $e', AppTheme.error);
    }
  }
  ...
  Future<void> _markRead() async {
    ...
    // V2: müşteri mesajlarını backend tarafında okundu olarak işaretle.
    unawaited(_api.markSupportTicketReadV2(id).catchError((_) =>
        <String, dynamic>{}));
  }
  ...
  Future<void> _refreshActive({required bool fromPolling}) async {
    ...
    // V2 poll'u paralel başlat (typing/durum sinyali için).
    unawaited(_pollV2TypingAndStatus(id));
    try {
  ...
  Future<void> _pollV2TypingAndStatus(int ticketId) async {
    try {
      final result = await _api.pollSupportTicketV2(
        ticketId,
        sinceMessageId: _lastKnownMessageId(),
      );
      if (!mounted || _toInt(_active?['id']) != ticketId) return;
      final data = result['data'];
      if (data is! Map) return;
      final isTyping = data['is_typing'] == true || data['is_typing'] == 1;
      if (isTyping != _peerTyping) {
        setState(() => _peerTyping = isTyping);
      }
    } catch (_) {
      // Endpoint kapalıysa veya auth düşerse sessizce yut.
    }
  }
              ...
              final actions = Wrap(
                ...
                children: [
                  ...
                  if (_selectionMode && _selectedIds.isNotEmpty)
                    actionButton(
                      onPressed: _bulkBusy ? () {} : _confirmBulkDelete,
                      icon: _bulkBusy
                          ? const SizedBox(
                              width: 16,
                              height: 16,
                      ...
                      tooltip: 'Seçilenleri Sil (${_selectedIds.length})',
                    ),
                  if (!compact)
                    actionButton(
                      onPressed: () => setState(
                        ...
                        itemBuilder: (context, i) {
                          ...
                          return InkWell(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: selected
                                    ? AppTheme.error.withValues(alpha: .14)
                                    : active
                                ...
                                border: Border.all(
                                    color: selected
                                        ? AppTheme.error.withValues(alpha: .55)
                                        : active
                                            ? AppTheme.primary
                                                .withValues(alpha: .55)
