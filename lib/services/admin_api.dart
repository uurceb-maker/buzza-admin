...
class AdminApi {
    ...
    for (final uri
      ...
      try {
        ...
        lastError = ApiException(
            'Sunucu yanıt vermiyor (45 sn zaman aşımı). Lütfen tekrar deneyin.');
        continue;
      } on http.ClientException catch (error) {
        lastError = ApiException('Bağlantı hatası: ${error.message}');
        continue;
      } catch (error) {
      ...
      if (response.statusCode == 429) {
        ...
        if (retryAfter != null) {
          ...
          if (secs != null) waitMinutes = (secs / 60).ceil();
        }
        String msg = 'Çok fazla deneme. $waitMinutes dakika sonra tekrar deneyin.';
        if (decoded is Map<String, dynamic>) {
          final serverMsg = '${decoded['error'] ?? decoded['message'] ?? ''}'.trim();
          if (serverMsg.isNotEmpty) msg = serverMsg;
        }
        throw RateLimitException(msg, retryMinutes: waitMinutes);
      }
  ...
  // ignore: unused_element — Ticket thread detay yüklemesi için planlanmış
  ...
  // ─── BUZZA DESTEK V2 — /wp-json/buzza/v1/ ──────────────────────────────────
  // Modern REST API: bulk delete, mesaj silme, poll, typing, multipart reply.
  // Auth: Operatör Bearer token'ı (Authorization header) zaten _jsonHeaders'ta
  // ekleniyor. Backend BD_Security::authenticate_bearer ile decode eder.

  Uri _buzzaV2Uri(String path, [Map<String, String>? query]) {
    final root = _siteRootUrl();
    final p = path.startsWith('/') ? path : '/$path';
    final uri = Uri.parse('$root/wp-json/buzza/v1$p');
  ...
  Future<Map<String, dynamic>> _buzzaV2Request(
    String method,
    String path, {
    Map<String, String>? query,
    Map<String, dynamic>? body,
  }) async {
    final uri = _buzzaV2Uri(path, query);
    ...
    try {
      ...
      throw ApiException('Baglanti hatasi: ${error.message}');
    } catch (error) {
      throw ApiException('Baglanti hatasi: $error');
    }
  ...
  /// `DELETE /buzza/v1/tickets/bulk` — Seçili biletleri ve fiziksel eklerini siler.
  Future<Map<String, dynamic>> bulkDeleteSupportTickets(
    List<int> ticketIds,
  ) async {
    if (ticketIds.isEmpty) {
      return <String, dynamic>{'success': true};
    }
    return _buzzaV2Request(
      'DELETE',
      '/tickets/bulk',
      body: <String, dynamic>{'ticket_ids': ticketIds},
    );
  }

  /// `DELETE /buzza/v1/tickets/{id}/messages` — Seçili mesajları (ve eklerini) siler.
  Future<Map<String, dynamic>> deleteSupportMessages(
    int ticketId,
    List<int> messageIds,
  ) async {
  ...
  /// `DELETE /buzza/v1/tickets/{id}` — Tek bir bileti (ve eklerini) siler.
  Future<Map<String, dynamic>> deleteSupportTicket(int ticketId) {
    return _buzzaV2Request('DELETE', '/tickets/$ticketId');
  }

  /// `GET /buzza/v1/tickets/{id}/poll?since=` — yeni mesaj/typing/durum
  Future<Map<String, dynamic>> pollSupportTicketV2(
    int ticketId, {
    int sinceMessageId = 0,
  }) {
    return _buzzaV2Request(
      'GET',
      '/tickets/$ticketId/poll',
      query: <String, String>{'since': '$sinceMessageId'},
    );
  }

  /// `POST /buzza/v1/tickets/{id}/typing` — Yazıyor sinyali (4 sn geçerli)
  Future<Map<String, dynamic>> sendSupportTypingSignalV2(int ticketId) {
    return _buzzaV2Request('POST', '/tickets/$ticketId/typing');
  }

  /// `POST /buzza/v1/tickets/{id}/read` — Müşteri mesajlarını okundu işaretle
  Future<Map<String, dynamic>> markSupportTicketReadV2(int ticketId) {
    return _buzzaV2Request('POST', '/tickets/$ticketId/read');
  }

  /// `POST /buzza/v1/tickets/{id}/reply` — Multipart yanıt (mesaj + opsiyonel dosya)
  /// Backend `$_FILES['file']` (tekil) bekler.
  Future<Map<String, dynamic>> replySupportTicketWithFile({
    ...
    final uri = _buzzaV2Uri('/tickets/$ticketId/reply');
    final request = http.MultipartRequest('POST', uri);
    final base = _jsonHeaders();
    base.remove('Content-Type'); // Multipart kendi sınırlandırıcısını üretir
    request.headers.addAll(base);
    ...
    try {
      ...
      if (body.trim().isEmpty) {
        if (streamed.statusCode >= 400) {
          throw ApiException('HTTP ${streamed.statusCode}');
        }
        return <String, dynamic>{'success': true};
      }
      final decoded = json.decode(body);
  ...
  Future<Map<String, dynamic>> getAutoOrders({
    int page = 1,
    int perPage = 20,
    String status = '',
    int userId = 0,
    ...
    return _request('/auto-orders?$query');
  }

  Future<Map<String, dynamic>> getAutoOrderDetail(int id) =>
      _request('/auto-orders/$id');

  Future<Map<String, dynamic>> autoOrderAction(int id, String action) =>
      _request('/auto-orders/$id/action',
          method: 'POST', body: {'action': action});

  Future<Map<String, dynamic>> createAutoOrder({
    required int userId,
    required int serviceId,
    required String targetLink,
    required int totalQuantity,
    required int perRunQuantity,
    required int runInterval,
  }) {
    return _request('/auto-orders', method: 'POST', body: {
      'user_id': userId,
      'service_id': serviceId,
      'target_link': targetLink,
      'total_quantity': totalQuantity,
      'per_run_quantity': perRunQuantity,
      'run_interval': runInterval,
    });
  }

  Future<Map<String, dynamic>> getAutoOrderStats() =>
      _request('/auto-orders/stats');

  Future<Map<String, dynamic>> getAutoOrderServices() =>
      _request('/auto-orders/services');

  Future<Map<String, dynamic>> getAutoOrderIntervals() =>
      _request('/auto-orders/intervals');
}
...
class RateLimitException extends ApiException {
  ...
  RateLimitException(super.message, {this.retryMinutes = 15});
}
