...
const Color _premiumCyan = Color(0xFFFDE047); // Light Gold
const Color _premiumViolet = Color(0xFFF59E0B); // Amber
const Color _premiumMint = Color(0xFFFACC15); // Buzza Yellow
    ...
    return Stack(
      children: [
        ...
        const Positioned.fill(
          child: IgnorePointer(
            child: RepaintBoundary(child: _DashboardNoiseOverlay()),
          ),
        ),
        child,
      ],
    ...
    return _GlassPanel(
      ...
      gradient: LinearGradient(
        colors: [
          ...
          const Color(0xFF09090B).withValues(alpha: 0.76),
...
class _NoisePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Performans: 18px yerine 28px grid aralığı, sin/cos yerine sabit pattern
    for (int y = 0; y < size.height; y += 28) {
      for (int x = 0; x < size.width; x += 28) {
        final double alpha = 0.015 + ((x * 7 + y * 13) % 17) * 0.002;
...
class _GlassPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // BackdropFilter sadece enableBlur=true olan ana panellerde kullanılır
    // İç içe BackdropFilter GPU'yu çok zorlayan bir operasyondur
    if (enableBlur) {
      return ClipRRect(
        borderRadius: BorderRadius.circular(borderRadius),
